...
- The faint full-bleed background behind every page hero is the shared `.hero__bg` CSS class
  (`styles/hero.css`) — a `<div className="hero__bg" aria-hidden="true" />` using a CSS
  `background-image`, not an `<img>`. Keep it that way: an `<img>` there gets picked up by Lighthouse
  as the LCP element (it was, before this was fixed) even though it's a decorative, low-opacity
  background — a CSS background is never an LCP candidate. The actual hero photo (`home-hero.webp`,
  `concept-hero.webp`, …) is the real LCP element and should keep `fetchpriority="high"` plus a
  matching `preloadImage` passed to `<Layout>`.
...
- `fonts.css`    — local `@font-face` for all site fonts (Agrandir + self-hosted Oxanium/Dela Gothic
  One/Barlow woff2 in `public/fonts/`), imported in `Layout.astro` before `global.css`. No Google
  Fonts `<link>` — fonts were migrated off `fonts.googleapis.com` to remove a render-blocking
  cross-origin request; only the weights actually referenced in `src/` are shipped (e.g. Barlow ships
  400/700 only).
...
modules (stays in sync with dates/pricing/speakers/FAQ) and is linked from `<head>` via
`<link rel="alternate" type="text/plain">` in `Layout.astro`. `vercel.json`
also keeps 301 redirects from the old `/<page>-2026` URLs (`/agenda-2026`, `/concept-2026`,
`/intervenants-2026`) to their clean equivalents, and sets a 1-year immutable `Cache-Control` on
`/assets/*` and `/fonts/*` (unhashed paths — the default Vercel caching doesn't cover them).
